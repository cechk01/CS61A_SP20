...
def choose(paragraphs, select, k):
	...
	# BEGIN PROBLEM 1
	"*** YOUR CODE HERE ***"
	filtered = [x for x in paragraphs if select(x)]
	if k >= len(filtered):
		return ''
	return filtered[k]
	# END PROBLEM 1
...
def autocorrect(user_word, valid_words, diff_function, limit):
	...
	if user_word in valid_words:
		return user_word
	min_diff_word = min(valid_words,key=lambda w: diff_function(user_word,w,limit))
...
def feline_fixes(start, goal, limit):
	...
	if limit < 0: # Fill in the condition
		...
		if start[0] == goal[0]: # Fill in the condition PB: letters match move to next letter
			# BEGIN
...
def time_per_word(times_per_player, words):
	...
	for time in times_per_player:
		tp = []
		for x in range(len(times_per_player[0])-1):
			tp.append(time[x+1] - time[x])
		times.append(tp)
...
def fastest_words(game):
	...
	for i in words: 
		...
		p = 0
		for j in players:
			if time(game,j,i) < lowest:
				p = j
				lowest = time(game,j,i)
		fastest_words_list[p].append(word_at(game,i))
